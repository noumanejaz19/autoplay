...
export default async function TeamPage() {
  ...
  if (user) {
    const { data } = await supabase.from("profiles").select("id, role").eq("auth_user_id", user.id).single();
    isAdmin = (data as { id: string; role: string } | null)?.role === "admin";
    currentProfileId = (data as { id: string; role: string } | null)?.id ?? null;
  }
